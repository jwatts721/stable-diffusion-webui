...
def git_clone(url, dir, name, commithash=None):
    ...
    if os.path.exists(dir):
        ...
        try:
            run_git(dir, name, 'fetch', f"Fetching updates for {name} => {url}", f"Couldn't fetch {name}", autofix=False)
            run_git(dir, name, f'checkout {commithash}', f"Checking out commit for {name} with hash: {commithash}...", f"Couldn't checkout commit {commithash} for {name}", live=True)

        except Exception:
            print(f"Skipping fetch for {name} — remote unavailable (patched) => {url}")
...
def prepare_environment():
    ...
    print("Repo directories A1111 is checking:")
    for name in ["stable-diffusion-stability-ai", "taming-transformers", "k-diffusion", "generative-models"]:
        print("  ", name, "=>", repo_dir(name), "exists:", os.path.exists(repo_dir(name)))

    # Add repos to sys.path so that their modules can be imported by extensions' install.py scripts
    sys.path.extend([repo_dir(name) for name in ["stable-diffusion-stability-ai", "taming-transformers", "k-diffusion", "generative-models"]])  
